...
// Extract day's range from TradingView text
...
// Extract change from TradingView text
function extractChange(text: string): { change: number; changePercent: number } {
  const match = text.match(/[−\-]([\d,.]+)\s*[−\-]([\d,.]+)%/);
  if (match) {
    return { change: -parseFloat(match[1].replace(/,/g, '')), changePercent: -parseFloat(match[2].replace(/,/g, '')) };
  }
  const posMatch = text.match(/\+([\d,.]+)\s*\+?([\d,.]+)%/);
  if (posMatch) {
    return { change: parseFloat(posMatch[1].replace(/,/g, '')), changePercent: parseFloat(posMatch[2].replace(/,/g, '')) };
  }
...
export async function fetchRealPrice(pair: string): Promise<MarketData> {
  ...
  // Strategy 2: Web search targeting TradingView ONLY
  try {
    const zai = await getZAI();
